...
6. **Append (or replace) in the /review comment.**
   ...
   replacement of any existing `pr-diagrams` block, and falling back to a new
   comment if needed:
   ...
   - If found and `--mode=append`: updates that comment via `gh api -X PATCH /repos/{owner}/{repo}/issues/comments/{id}`. Replaces any prior `pr-diagrams:begin/end` block in place; otherwise appends the new block to the bottom.
...
- **Never** post more than one `pr-diagrams` block per PR — always replace the previous one.
